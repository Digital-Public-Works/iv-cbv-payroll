...
| `SUPPORTED_PROVIDERS` | `/service/app-{env}/supported-providers` | String | Comma-separated payroll aggregators | `argyle,pinwheel` (prod), `argyle` (demo) |
...
- Visit application URL and confirm it loads
...
- Verify all SecureString parameters are accessible by ECS task role
...
- Check webhook configuration (see [Webhook Configuration](../webhook-configuration.md))
- Verify environment settings match intended API endpoints
