...
RSpec.describe Api::InvitationsController do
  ...
  describe "#create" do
    ...
    context "with optional expiration params" do
      let(:valid_expiration_date) { 30.days.from_now }
      ...
      context "when only a valid expiration date is provided" do
        ...
        it "creates an invitation with the provided expiration date" do
          invitation = CbvFlowInvitation.last
          actual_expires_at = invitation.expires_at.in_time_zone(agency_config.timezone)
          expected_expires_at = valid_expiration_date.to_date.in_time_zone(agency_config.timezone).end_of_day
      ...
      context "that are invalid" do
        context "with a date in the past" do
          let(:expiration_params) { { expiration_date: 3.days.ago } }
        ...
        context "with a date too far in the future" do
          let(:expiration_params) { { expiration_date: 367.days.from_now } }
        ...
        context "with an invalid date format" do
          ...
          it "returns an error with a descriptive message" do
            expect(response).to have_http_status(:unprocessable_entity)
            parsed_response = JSON.parse(response.body)
            expect(parsed_response["errors"][0]["message"]).to include("is not a valid date format")
          end
        end
      end

      it "is computed using the agency's timezone" do
