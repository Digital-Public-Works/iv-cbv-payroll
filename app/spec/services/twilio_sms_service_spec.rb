...
# The Twilio path is currently unused (invitations go out over AWS SNS), but
# these specs keep it honest in case we return to it.
RSpec.describe TwilioSmsService, type: :service do
  ...
  it "sends through the configured messaging service and returns the message id" do
    ...
    expect(service.send_message(to: "+15552345678", body: "hello")).to eq("SMtest")
  ...
  it "raises PermanentDeliveryError for non-retryable Twilio codes" do
    described_class::PERMANENT_ERROR_CODES.each do |code|
      ...
      expect { service.send_message(to: "+15552345678", body: "hello") }
        .to raise_error(SmsService::PermanentDeliveryError) { |error|
  ...
  it "raises a retryable DeliveryError for other Twilio codes" do
    ...
    expect { service.send_message(to: "+15552345678", body: "hello") }
      .to raise_error(SmsService::DeliveryError) { |error|
        expect(error).not_to be_a(SmsService::PermanentDeliveryError)
