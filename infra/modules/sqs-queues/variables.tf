...
variable "use_environment_suffix" {
  type        = bool
  default     = false
  description = "Whether to append environment name to queue names. Set to true for non-standard environments (a11y) to avoid resource conflicts. Standard environments (demo, prod) keep original names for backward compatibility."
}
